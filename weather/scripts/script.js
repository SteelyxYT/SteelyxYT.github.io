...
const options = {
  ...
  fetch(
    ...
    .then((json) => {
      ...
      console.log(`https://api.met.no/weatherapi/locationforecast/2.0/complete?lat=${currentPosition.coords.latitude}&lon=${currentPosition.coords.longitude}&altitude=${Math.round(currentPosition.coords.altitude)}`);
      console.log(json);
      timetable.forEach((element) => {
        ...
        document.getElementById("jsOutput").innerHTML += `<h2><strong>Time: ${date.toLocaleDateString("nb-NO", options)}</strong></h2><br>lufttrykk på sjø nivå: ${element.data.instant.details.air_pressure_at_sea_level}${Units.air_pressure_at_sea_level}<br>luft temperatur: ${element.data.instant.details.air_temperature} ${Units.air_temperature}<br>skyområdefraksjon: ${element.data.instant.details.cloud_area_fraction}${Units.cloud_area_fraction}<br>relativ fuktighet: ${element.data.instant.details.relative_humidity}${Units.relative_humidity}<br>vind fra retning: ${element.data.instant.details.wind_from_direction}${Units.wind_from_direction}<br>vindhastighet: ${element.data.instant.details.wind_speed}${Units.wind_speed}<br><br><strong>Neste time</strong><br>nedbør: ${element.data.next_1_hours.details.precipitation_amount}${Units.precipitation_amount}<br>sjanse av nedbør: ${element.data.next_1_hours.details.probability_of_precipitation}${Units.probability_of_precipitation}<br>sjanse av lyn: ${element.data.next_1_hours.details.probability_of_thunder}${Units.probability_of_thunder}<br>vær: ${element.data.next_1_hours.summary.symbol_code.replace(/_/g, ' ')}<br><img src="./icons/${element.data.next_1_hours.summary.symbol_code}.svg" alt="${element.data.next_1_hours.summary.symbol_code}"><br><br><strong>Neste 6 timer</strong><br>nedbør: ${element.data.next_6_hours.details.precipitation_amount}${Units.precipitation_amount}<br>sjanse av nedbør: ${element.data.next_6_hours.details.probability_of_precipitation}${Units.probability_of_precipitation}<br>sjanse av lyn: ${element.data.next_6_hours.details.probability_of_thunder}${Units.probability_of_thunder}<br>vær: ${element.data.next_6_hours.summary.symbol_code.replace(/_/g, ' ')}<br><img src="./icons/${element.data.next_6_hours.summary.symbol_code}.svg" alt="${element.data.next_6_hours.summary.symbol_code}"><br><br><strong>Neste 12 timer</strong><br>nedbør: ${element.data.next_12_hours.details.precipitation_amount}${Units.precipitation_amount}<br>sjanse av nedbør: ${element.data.next_12_hours.details.probability_of_precipitation}${Units.probability_of_precipitation}<br>sjanse av lyn: ${element.data.next_12_hours.details.probability_of_thunder}${Units.probability_of_thunder}<br>vær: ${element.data.next_12_hours.summary.symbol_code.replace(/_/g, ' ')}<br><img src="./icons/${element.data.next_12_hours.summary.symbol_code}.svg" alt="${element.data.next_12_hours.summary.symbol_code}"><br><br>`
      });
